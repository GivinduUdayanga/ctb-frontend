...
export default  function AdminLocalTeaNewsPage() {
    const [localTeaNews, setLocalTeaNews] = useState([]);

        useEffect(() => {
        axios.get("http://localhost:3000/api/admin/local-tea-news")
            .then(response => {
                console.log(response.data);
                setLocalTeaNews(response.data);
            })
            .catch(error => {
                console.error("Error fetching local tea news data:", error);
            });
    }, [])
                                    ...
                                    <button 
                                        onClick={
                                            ()=> {
                                                ...
                                                axios.delete(`http://localhost:3000/api/admin/local-tea-news/${item.newsID}` , {
                                                    headers: {
                                                        Authorization: `Bearer ${token}`
                                                    }
                                                }).then (
                                                    ()=> {
                                                        toast.success("local tea news deleted sucessfully ...");
                                                        setLocalTeaNews(
                                                            localTeaNews.filter(report => report.newsID !== item.newsID)
                                                        );
                                                    }
                                                ).catch((error) => {
                                                    console.log(error);
