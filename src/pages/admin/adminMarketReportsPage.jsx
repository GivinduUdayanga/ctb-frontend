...
export default  function AdminMarketReportsPage() {
    ...
    useEffect(() => {
        axios.get("http://localhost:3000/api/admin/market-reports")
            .then(response => {
                console.log(response.data);
                setMarketReports(response.data);
            })
            .catch(error => {
                console.error("Error fetching market reports data:", error);
            });
    }, [])
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-sm text-left text-black">
                    ...
                    <thead className="h-[50px] text-xs uppercase bg-black text-gray-50 border-b border-white/10">
                        <tr>
                            <th className="py-3 px-5 border-white border-r-[1px]">Image</th>
                            <th className="py-3 px-5 border-white border-r-[1px]">Report ID</th>
                            <th className="py-3 px-5 border-white border-r-[1px]">PDF</th>
                            <th className="py-3 px-5 border-white border-r-[1px]">Year</th>
                            <th className="py-3 px-5 border-white border-r-[1px]">Start Date</th>
                            <th className="py-3 px-5 border-white border-r-[1px]">Header</th>
                            <th className="py-3 px-5 border-white border-r-[1px]">Title</th>
                                    ...
                                    <button 
                                        onClick={
                                            ()=> {
                                                ...
                                                axios.delete(`http://localhost:3000/api/admin/market-reports/${item.reportID}` , {
                                                    headers: {
                                                    ...
                                                    ()=> {
                                                        ...
                                                        setMarketReports(
                                                            marketReports.filter(report => report.reportID !== item.reportID)
                                                        );
                                                    }
                                                ).catch((error) => {
                                                    console.log(error);
