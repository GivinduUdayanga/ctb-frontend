...
export default function AdminUpdateGalleryPage () {
    ...
    async function updateGalleryItem() {
        ...
        if (token == null) {
            toast.error("You must be logged in as an admin to update gallery items.");
            navigate("/login");
            return;
        }
        ...
        try {
            ...
            await axios.put("http://localhost:3000/api/admin/gallery/" + galleryID, {
            ...
            toast.success("New gallery item updated successfully");
            navigate("/admin/gallery");

        } catch (error) {
            toast.error("Error updating new gallery item. Please try again.");
            console.log("Error updating new gallery item:");
            console.log(error);
        }
    ...
    return (
        <div className="w-full h-full flex justify-center p-[50px] items-start text-black overflow-y-scroll">
            <div className="w-[800px] bg-gray-300 p-[30px] rounded-2x slhadow-2xl overflow-y-visible">
                ...
                <h1 className="w-full text-2xl font-bold mb-[20px] flex items-center gap-[10px]"><GrGallery />Update Gallery Item</h1>
                ...
                <div className="w-full bg-white p-[20px] flex flex-row flex-wrap justify-between rounded-xl">
                    ...
                    <div className="my-[10px] w-[30%]">
                        ...
                        <input disabled type="text" value={galleryID} onChange={(e) => {setGalleryID(e.target.value)}} className="w-full h-[30px] rounded border-2 shadow-2xl text-black px-[20px] focus:outline-none focus:ring-1 focus:ring-green-500" />
                    </div>
