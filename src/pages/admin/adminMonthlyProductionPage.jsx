...
export default  function AdminMonthlyProductionPage() {
        ...
        <div className="w-full max-h-[620px] border-[1px] rounded-lg bg-gray-300 flex flex-col p-8">
            ...
            <div>
                <h1 className="flex flex-row items-center text-3xl font-bold mb-10 gap-[13px]">
                    <AiOutlineProduct /> Monthly Production
                </h1> 
            </div>
                ...
                <table className="w-full text-sm text-left text-black border-gray-400">
                    ...
                    <thead className="h-[50px] text-xs uppercase bg-black text-gray-50  border-b border-white/10">
                        <tr>
                            ...
                            <th className="px-5 py-3 border-white border-r-[1px]">OHM</th>
                            <th className="px-5 py-3 border-white border-r-[1px]">OMM</th>
                            <th className="px-5 py-3 border-white border-r-[1px]">CMM</th>
                            <th className="px-5 py-3 border-white border-r-[1px]">CLM</th>
                            <th className="px-5 py-3 border-white border-r-[1px]">GTHM</th>
                            <th className="px-5 py-3 border-white border-r-[1px]">GTLM</th>
                        ...
                        {monthlyProduction.map((item, index) => (
                            ...
                            >
                                ...
                                <td className="px-5 py-2">
                                    {item.orthodoxHighMonth}
                                </td>
                                <td className="px-5 py-2"> 
                                    {item.orthodoxMediumMonth}
                                </td>
                                <td className="px-5 py-2">
                                    {item.ctcMediumMonth} 
                                </td>
                                ...
                                <td className="px-5 py-2">
                                    {item.greenTeaHighMonth}
                                </td>
                                <td className="px-5 py-2">
                                    {item.greenTeaLowMonth}
                                </td>
                                    ...
                                    <button 
                                        onClick={
                                            ()=> {
                                                ...
                                                axios.delete(`http://localhost:3000/api/admin/monthly-production/${item.productionID}` , {
                                                    headers: {
                                                        Authorization: `Bearer ${token}`
                                                    }
                                                }).then (
                                                    ()=> {
                                                        toast.success("monthly Production deleted sucessfully ...");
                                                        setMonthlyProduction(
                                                            monthlyProduction.filter(report => report.productionID !== item.productionID)
                                                        );
                                                    }
                                                ).catch((error) => {
                                                    console.log(error);
            ...
            <Link 
                to="/admin/add-monthly-production"
