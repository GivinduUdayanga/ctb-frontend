...
export default function AdminAddInternationalNewsPage () {
        ...
        axios.get("http://localhost:3000/api/admin/international-news")
            .then((response) => {
                ...
                if (response.data.length > 0) {
                    ...
                    const lastID = response.data[0].newsID;
                    setNewsID(lastID + 1);
            ...
            .catch((error) => {
                ...
                console.log("Error fetching news ID:", error);
    ...
    async function addInternationalNews() {
        const token = localStorage.getItem("token");
        if (token == null) {
            toast.error("You must be logged in as an admin to add international news.");
            navigate("/login");
            return;
        }

        if(image==="" || title==="" || fullDescription==="" || shortDescription==="") {
        ...
        try {
            ...
            await axios.post("http://localhost:3000/api/admin/international-news", {
                image: image,
                title: title,
            ...
            navigate("/admin/international-news");

        } catch (error) {
            toast.error("Error adding international news. Please try again.");
            console.log("Error adding international news:");
            console.log(error);
        }
    ...
    return (
        <div className="w-full h-full flex justify-center p-[50px] items-start text-black overflow-y-scroll">
            <div className="w-[800px] bg-gray-300 p-[30px] rounded-2xl shadow-2xl overflow-y-visible">
                ...
                <h1 className="w-full text-2xl font-bold mb-[20px] flex items-center gap-[10px]"><LuNewspaper />Add New International News</h1>
                ...
                <div className="w-full bg-white p-[20px] flex flex-row flex-wrap justify-between rounded-xl">
                    <div className="my-[10px] w-[40%]">
                        ...
                        <input type="number" value={newsID} disabled className="w-full h-[30px] rounded border-2 shadow-2xl text-black bg-gray-100 px-[20px] cursor-not-allowed focus:outline-none" />
                    ...
                    <Link to="/admin/international-news"
                    className="w-[45%]">
