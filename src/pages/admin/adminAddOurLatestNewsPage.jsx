...
export default function AdminAddOurLatestNewsPage () {
    ...
    useEffect(() => {
        ...
        axios.get("http://localhost:3000/api/admin/our-latest-news/next-id")
            .then((response) => {
                setNewsID(response.data.nextID);
            })
            .catch((error) => {
    ...
    async function addOurLatestNews() {
        const token = localStorage.getItem("token");
        if (token == null) {
            toast.error("You must be logged in as an admin to add our latest news.");
            navigate("/login");
            return;
        }
        ...
        try {
            ...
            await axios.post("http://localhost:3000/api/admin/our-latest-news/", {
                newsID: newsID,
                image: image,
                title: title,
            ...
            toast.success("Our latest news added successfully");
            navigate("/admin/our-latest-news");

        } catch (error) {
            toast.error("Error adding our latest news. Please try again.");
            console.log("Error adding our latest news:");
            console.log(error);
        }
    ...
    return (
        <div className="w-full h-full flex justify-center p-[50px] items-start text-black overflow-y-scroll">
            <div className="w-[800px] bg-gray-300 p-[30px] rounded-2xl shadow-2xl overflow-y-visible">
                ...
                <h1 className="w-full text-2xl font-bold mb-[20px] flex items-center gap-[10px]"><LuNewspaper />Add New Our Latest News</h1>
                ...
                <div className="w-full bg-white p-[20px] flex flex-row flex-wrap justify-between rounded-xl">
                    <div className="my-[10px] w-[40%]">
                        ...
                        <input
                            ...
                            value={newsID}
                            disabled
                            className="w-full h-[30px] rounded border-2 shadow-2xl bg-gray-200 text-black px-[20px] focus:outline-none"
                        />
                    </div>
