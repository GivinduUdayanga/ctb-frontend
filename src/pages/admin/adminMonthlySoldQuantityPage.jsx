...
export default  function AdminMonthlySoldQuantityPage() {

    const [monthlySoldQuantity, setMonthlySoldQuantity] = useState([]);

        useEffect(() => {
        axios.get("http://localhost:3000/api/admin/monthly-sold-quantity")
            .then(response => {
                console.log(response.data);
                setMonthlySoldQuantity(response.data);
            })
            .catch(error => {
                console.error("Error fetching monthly sold quantity data:", error);
            });
    }, [])
        ...
        <div className="w-full max-h-[620px] border-[1px] rounded-lg bg-gray-300 flex flex-col p-8">
            ...
            <div>
                <h1 className="flex flex-row items-center text-3xl font-bold mb-10 gap-[13px]">
                    <BsBarChartLine />Monthly Sold Quantity & Averages
                </h1> 
            </div>
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-sm text-left text-black border-gray-400">
                    ...
                    {/* Body */}
                    <tbody>
                        {monthlySoldQuantity.map((item, index) => (
                            <tr
                                key={index}
                                    ...
                                    <button 
                                        onClick={
                                            ()=> {
                                                ...
                                                axios.delete(`http://localhost:3000/api/admin/monthly-sold-quantity/${item.quantityID}` , {
                                                    headers: {
                                                        Authorization: `Bearer ${token}`
                                                    }
                                                }).then (
                                                    ()=> {
                                                        toast.success("monthly sold quantity deleted sucessfully ...");
                                                        setMonthlySoldQuantity(
                                                            monthlySoldQuantity.filter(report => report.quantityID !== item.quantityID)
                                                        );
                                                    }
                                                ).catch((error) => {
                                                    console.log(error);
            ...
            <Link 
                to="/admin/add-monthly-sold-quantity"
