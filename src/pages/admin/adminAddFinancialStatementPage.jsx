...
export default function AdminAddFinancialStatementPage () {
    ...
    useEffect(() => {
        ...
        axios.get("http://localhost:3000/api/admin/financial-statement/next-id")
            .then((response) => {
    ...
    async function addFinancialStatement() {
        const token = localStorage.getItem("token");
        if (token == null) {
            toast.error("You must be logged in as an admin to add a financial statement.");
            navigate("/login");
            return;
        }

        if(reportID==="" || image==="" || title==="" || date==="" || pdf==="") {
        ...
        try {
            ...
            await axios.post("http://localhost:3000/api/admin/financial-statement/", {
                reportID: reportID,
                image: image,
                title: title,
                date: date,
            ...
            navigate("/admin/financial-statement");

        } catch (error) {
            toast.error("Error adding financial statement. Please try again.");
            console.log("Error adding financial statement:");
            console.log(error);
        }
    ...
    return (
        <div className="w-full h-full flex justify-center p-[50px] items-start text-black overflow-y-scroll">
            <div className="w-[800px] bg-gray-300 p-[30px] rounded-2xl shadow-2xl overflow-y-visible">
                ...
                <h1 className="w-full text-2xl font-bold mb-[20px] flex items-center gap-[10px]"><FaRegFileLines />Add New Financial Statement</h1>
                ...
                <div className="w-full bg-white p-[20px] flex flex-row flex-wrap justify-between rounded-xl">
                    ...
                    <div className="my-[10px] w-[40%]">
                        ...
                        <input type="date" value={date} onChange={(e) => {setDate(e.target.value)}} className="w-full h-[30px] rounded border-2 shadow-2xl text-black px-[20px] focus:outline-none focus:ring-1 focus:ring-green-500"  />
                    ...
                    <Link to="/admin/financial-statement"
                    className="w-[45%]">
